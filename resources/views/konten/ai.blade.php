...
@section('content')
    <div x-data="{ historyOpen: false }"
        class="bg-white font-['Radio_Canada'] min-h-screen flex flex-col items-center justify-center relative px-6 py-12">
        <!-- Top Action Buttons -->
        <div class="absolute top-8 left-0 right-0 px-8 md:px-12 flex justify-between items-center w-full">
            <!-- Back Button -->
            <a href="{{ url('/home') }}"
                class="w-14 h-14 bg-white shadow-[0_2px_15px_rgba(0,0,0,0.06)] rounded-full flex items-center justify-center hover:bg-gray-50 transition">
                <svg class="w-6 h-6 text-[#4C732E]" fill="none" stroke="currentColor" stroke-width="2.5"
                    viewBox="0 0 24 24">
            ...
            <button @click="historyOpen = true"
                class="w-14 h-14 bg-white shadow-[0_2px_15px_rgba(0,0,0,0.06)] rounded-full flex items-center justify-center hover:bg-gray-50 transition">
                <svg class="w-6 h-6 text-[#4C732E]" fill="none" stroke="currentColor" stroke-width="2"
                    viewBox="0 0 24 24">
                    <rect x="3" y="6" width="18" height="12" rx="2" stroke="currentColor" stroke-width="2"
                        fill="none"></rect>
                    <line x1="16" y1="6" x2="16" y2="18" stroke="currentColor"
                        stroke-width="2"></line>
                </svg>
            </button>
        </div>
        ...
        <div class="flex flex-col items-center w-full max-w-3xl mt-12 md:mt-2">
            ...
            <div id="actionButtons" class="flex flex-col md:flex-row gap-5 w-full justify-center mb-8 transition-all">
                ...
                <button type="button" onclick="document.getElementById('cameraInput').click()"
                    class="w-full md:w-auto px-10 py-3.5 bg-[#4C732E] text-white text-lg md:text-xl rounded-full hover:bg-[#3b5924] transition flex items-center justify-center font-semibold">
                    Ambil Foto
                </button>

                {{-- Tombol Upload --}}
                <button type="button" onclick="document.getElementById('galleryInput').click()"
                    class="w-full md:w-auto px-10 py-3.5 bg-white text-[#717171] text-lg md:text-xl rounded-full border-[2.5px] border-[#4C732E] hover:bg-gray-50 transition flex items-center justify-center font-semibold">
                    Upload dari Galeri
                </button>
            </div>
            ...
            <div id="previewContainer" class="hidden flex-col items-center w-full max-w-sm mb-8 transition-all">
                <img id="previewImage" src="" alt="Preview Gambar"
                    class="w-full aspect-square object-cover rounded-3xl shadow-lg border-[3px] border-[#4C732E] mb-5">

                <div class="flex gap-4 w-full justify-center">
                    {{-- Tombol Ulangi --}}
                    <button type="button" id="btnRetry"
                        class="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center hover:bg-red-100 transition shadow-sm border border-red-200">
                    ...
                    <button type="button" id="btnUpload"
                        class="flex-1 px-8 py-4 bg-[#4C732E] text-white text-xl rounded-full hover:bg-[#3b5924] transition flex items-center justify-center font-bold shadow-sm">
                        Lanjut
            ...
            <div id="uploadStatus" class="hidden flex-col items-center mb-6">
                <div id="loadingSpinner"
                    class="w-8 h-8 border-4 border-[#4C732E] border-t-transparent rounded-full animate-spin mb-2"></div>
                <p id="uploadText" class="text-[#4C732E] font-semibold text-center text-sm md:text-base">Mempersiapkan
                    Upload...</p>
            </div>
        ...
        <!-- History Drawer -->
        <div x-show="historyOpen" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0 translate-x-full" x-transition:enter-end="opacity-100 translate-x-0"
            x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 translate-x-0"
            x-transition:leave-end="opacity-0 translate-x-full"
            class="fixed inset-y-0 right-0 w-full md:w-96 bg-white shadow-2xl z-50 flex flex-col" style="display: none;">

            <!-- Header -->
            <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <div class="flex items-center gap-3">
                    <svg class="w-6 h-6 text-[#4C732E]" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <h3 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                        Riwayat Deteksi
                        <svg id="btn-refresh-history"
                            class="w-5 h-5 text-[#4C732E] cursor-pointer hover:text-green-700 transition" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" title="Refresh Riwayat">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                        </svg>
                    </h3>
                ...
                <button @click="historyOpen = false"
                    class="p-2 bg-white rounded-full text-gray-400 hover:text-gray-600 shadow-sm border border-gray-100 transition">
            ...
            <div id="history-list-container" class="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50/50 pb-20">
                @forelse($historyScans as $scan)
                    <a href="{{ route('ai.result', $scan->id) }}" id="history-card-{{ $scan->id }}"
                        class="relative block w-full bg-white border border-gray-100 p-4 rounded-xl shadow-sm hover:shadow-md hover:border-[#4C732E]/30 transition group flex flex-col gap-3">
                        <button onclick="deleteHistory(event, {{ $scan->id }})"
                            class="absolute top-2 right-2 p-2 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg md:opacity-0 group-hover:opacity-100 transition-all z-10 focus:opacity-100"
                            title="Hapus Riwayat">
                        ...
                        <div class="flex items-start gap-4 pr-6">
                            @php
                                $imgPath = $scan->image_path ?? '';
                                if (file_exists(public_path('uploads/' . $imgPath))) {
                                    $thumbUrl = url('uploads/' . $imgPath);
                                } elseif (file_exists(storage_path('app/public/' . $imgPath))) {
                                    $thumbUrl = url('storage/' . $imgPath);
                                } else {
                                    $thumbUrl = Storage::url($imgPath);
                                }
                            @endphp
                            <img src="{{ $thumbUrl }}" alt="{{ $scan->plant_name }}"
                                class="w-16 h-16 object-cover rounded-lg shadow-sm border border-gray-50"
                                onerror="this.src='{{ asset('image/placeholder-plant.jpg') }}'; this.onerror=null;">
                            <div class="flex-1 min-w-0">
                                <h4 class="font-bold text-gray-800 truncate group-hover:text-[#4C732E] transition">
                                    {{ $scan->plant_name }}</h4>
                                @if ($scan->ai_health_status == 'healthy')
                                    <span
                                        class="inline-flex items-center gap-1 mt-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-green-50 text-green-700 border border-green-200">
                                ...
                                @else
                                    <span
                                        class="inline-flex items-center gap-1 mt-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-red-50 text-red-700 border border-red-200">
                                        <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                            stroke-width="3">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div class="pt-2 border-t border-gray-50 flex justify-between items-center">
                            <span
                                class="text-xs font-medium text-gray-400">{{ $scan->created_at->diffForHumans() }}</span>
                            <span
                                class="text-xs font-semibold text-[#4C732E] flex items-center group-hover:translate-x-1 transition-transform">
                                Lihat Detail
        ...
        <div id="deleteConfirmModal"
            class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-[70] flex items-center justify-center transition-opacity duration-300 opacity-0">
            <div id="deleteConfirmModalContent"
                class="bg-white p-6 rounded-2xl shadow-2xl max-w-sm w-full mx-4 transform scale-95 transition-transform duration-300">
                <div class="flex flex-col items-center text-center">
                    ...
                    <p class="text-sm text-gray-500 mb-6">Apakah Anda yakin? Data dan foto deteksi ini akan dihapus secara
                        permanen.</p>
                    <div class="flex gap-3 w-full">
                        <button onclick="closeDeleteModal()"
                            class="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-gray-600 font-semibold hover:bg-gray-50 transition">Batal</button>
                        <button id="confirmDeleteBtn"
                            class="flex-1 px-4 py-2.5 rounded-xl bg-red-500 text-white font-semibold hover:bg-red-600 transition flex items-center justify-center">Hapus</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                async function uploadImage(file) {
                    ...
                    '{{ csrf_token() }}'); // CSRF token untuk keamanan form POST Laravel
                    ...
                    try {
                        ...
                        uploadText.className =
                            'text-red-500 font-semibold text-center text-sm md:text-base mt-2 block';
                        ...
                        fetch(`/ai/history/${id}`, {
                            ...
                            .then(data => {
                                if (data && data.status === 'success') {
                                    ...
                                    throw new Error((data && data.message) ? data.message :
                                        'Respon JSON gagal menghapus riwayat.');
                                }
                            })
                            .catch(error => {
