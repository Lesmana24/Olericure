...
@section('content')
    <div x-data="{ chatOpen: false }"
        class="bg-gray-50 font-['Radio_Canada'] min-h-screen py-6 px-4 flex justify-center items-center relative">

        {{-- Main Container --}}
        <div
            class="w-full max-w-6xl mx-auto bg-white shadow-xl rounded-2xl overflow-hidden flex flex-col md:flex-row md:h-[90vh]">
            ...
            <div class="w-full md:w-2/5 bg-gray-50 relative flex-shrink-0">
                ...
                <div class="absolute top-4 left-4 z-10">
                    @if (isset($isPreview) && $isPreview)
                        <form action="{{ route('ai.reset') }}" method="POST">
                            @csrf
                            <button type="submit"
                                class="w-10 h-10 bg-white/90 backdrop-blur-sm shadow-md rounded-full flex items-center justify-center hover:bg-gray-100 transition group text-gray-700">
                    ...
                    @else
                        <a href="{{ route('ai.index') }}"
                            class="w-10 h-10 bg-white/90 backdrop-blur-sm shadow-md rounded-full flex items-center justify-center hover:bg-gray-100 transition group text-gray-700 inline-flex">
                            <svg class="w-5 h-5 group-hover:-translate-x-1 transition-transform" fill="none"
                                stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                ...
                @php
                    // Smart image URL resolver untuk Shared Hosting (InfinityFree)
                    // Mencoba beberapa path agar tidak tergantung pada symlink atau config cache
                    $imagePath = $scanResult->image_path ?? '';
                    $imageUrl = null;

                    // Prioritas 1: Cek apakah file ada di public/uploads (konfigurasi baru)
                    if (file_exists(public_path('uploads/' . $imagePath))) {
                        $imageUrl = url('uploads/' . $imagePath);
                    }
                    // Prioritas 2: Cek apakah file ada di storage/app/public (konfigurasi lama)
                    elseif (file_exists(storage_path('app/public/' . $imagePath))) {
                        $imageUrl = url('storage/' . $imagePath);
                    }
                    // Prioritas 3: Fallback ke Storage::url() (biarkan Laravel handle sendiri)
                    else {
                        $imageUrl = Storage::url($imagePath);
                    }
                @endphp
                <img src="{{ $imageUrl }}" alt="Foto Tanaman"
                    class="w-full h-64 md:h-full object-cover rounded-t-2xl md:rounded-none"
                    onerror="this.src='{{ asset('image/placeholder-plant.jpg') }}'; this.onerror=null;">
            </div>
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="bg-gray-50 border border-gray-100 rounded-xl p-4">
                            <span class="block text-xs font-semibold text-gray-400 tracking-wider uppercase mb-1">Nama
                                Tanaman</span>
                            <div class="font-bold text-gray-900 text-xl capitalize mb-2 line-clamp-1">
                                {{ $scanResult->plant_name ?? 'Tanaman Tidak Diketahui' }}
                            </div>
                            <span
                                class="inline-block px-3 py-1 bg-yellow-100 text-yellow-800 rounded-md uppercase tracking-wider font-bold text-xs shadow-sm">
                        ...
                        <div class="bg-gray-50 border border-gray-100 rounded-xl p-4 flex flex-col justify-between">
                            <div class="mb-3">
                                <span
                                    class="block text-xs font-semibold text-gray-400 tracking-wider uppercase mb-1">Penyakit
                                    Utama</span>
                                ...
                                @else
                                    <span class="font-bold text-red-600 text-lg leading-tight block line-clamp-2"
                                        title="{{ $scanResult->disease_name }}">
                            ...
                            <div>
                                <span
                                    class="block text-xs font-semibold text-gray-400 tracking-wider uppercase mb-1">Akurasi
                                    Deteksi</span>
                                <span
                                    class="font-extrabold text-[#4C732E] text-xl">{{ number_format($scanResult->confidence_score, 1) }}%</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-start bg-slate-50 border border-slate-100 rounded-xl p-4">
                            ...
                            <div>
                                <span
                                    class="block text-xs font-bold text-[#4C732E] uppercase tracking-wider mb-1">Cahaya</span>
                                <p class="text-sm text-gray-600 leading-relaxed">
                                    {{ $scanResult->care_light ?? 'Informasi tidak tersedia' }}</p>
                            </div>
                        </div>
                        {{-- Card Penyiraman --}}
                        <div class="flex items-start bg-slate-50 border border-slate-100 rounded-xl p-4">
                            ...
                            <div>
                                <span
                                    class="block text-xs font-bold text-[#4C732E] uppercase tracking-wider mb-1">Penyiraman</span>
                                <p class="text-sm text-gray-600 leading-relaxed">
                                    {{ $scanResult->care_water ?? 'Informasi tidak tersedia' }}</p>
                            </div>
                        </div>
                        {{-- Card Suhu --}}
                        <div class="flex items-start bg-slate-50 border border-slate-100 rounded-xl p-4">
                            <span class="text-2xl mr-3 mt-0.5">🌡️</span>
                            <div>
                                <span class="block text-xs font-bold text-[#4C732E] uppercase tracking-wider mb-1">Suhu
                                    Ideal</span>
                                <p class="text-sm text-gray-600 leading-relaxed">
                                    {{ $scanResult->care_temperature ?? 'Informasi tidak tersedia' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <ul class="space-y-2 list-none">
                            @foreach ($scanResult->problems_list as $problem)
                                <li class="flex items-start text-sm text-orange-900 leading-relaxed">
                                    <span
                                        class="inline-block w-1.5 h-1.5 rounded-full bg-orange-400 mt-1.5 mr-2.5 flex-shrink-0"></span>
                                    {{ $problem }}
                                </li>
                            @endforeach
                        </ul>
                ...
                <div class="mt-auto pt-4 border-t border-gray-100 flex flex-col sm:flex-row gap-3">
                    @if (isset($isPreview) && $isPreview)
                        <form action="{{ route('ai.store') }}" method="POST" class="flex-1">
                            @csrf
                            <button type="submit"
                                class="w-full bg-[#4C732E] hover:bg-[#3b5924] text-white py-3.5 px-4 rounded-xl font-bold transition flex items-center justify-center shadow-md">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" stroke-width="2.5"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <form action="{{ route('ai.reset') }}" method="POST" class="flex-1">
                            @csrf
                            <button type="submit"
                                class="w-full bg-white hover:bg-gray-50 text-[#4C732E] border-2 border-[#4C732E] py-3.5 px-4 rounded-xl font-bold transition flex items-center justify-center shadow-sm">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" stroke-width="2.5"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div x-cloak>
            ...
            <div x-show="chatOpen" x-transition:enter="transition-opacity ease-linear duration-300"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                x-transition:leave="transition-opacity ease-linear duration-300" x-transition:leave-start="opacity-100"
                x-transition:leave-end="opacity-0" class="fixed inset-0 bg-black/50 z-40 backdrop-blur-sm"
                @click="chatOpen = false"></div>

            {{-- Slide-over Panel Kanan --}}
            <div class="fixed inset-y-0 right-0 w-full md:w-[450px] bg-white shadow-2xl transform transition-transform duration-300 ease-in-out z-50 flex flex-col"
                ...
                <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 flex-shrink-0">
                    <div class="flex items-center gap-4">
                        <div
                            class="w-12 h-12 rounded-full bg-[#4C732E]/10 flex items-center justify-center border border-[#4C732E]/20">
                        ...
                        <div>
                            ...
                            <div class="flex items-center mt-1">
                                ...
                                <span class="text-xs text-gray-500 font-semibold uppercase tracking-wider">Online
                                    (Botanist)</span>
                            </div>
                        </div>
                    </div>
                    <button @click="chatOpen = false"
                        class="text-gray-400 hover:text-red-500 hover:bg-red-50 p-2.5 rounded-full transition duration-200">
                ...
                <div id="chat-box" class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-white">
                    ...
                    <div class="text-center">
                        <span
                            class="text-xs font-semibold text-gray-400 bg-gray-50 px-3 py-1 rounded-full border border-gray-100">Hari
                            ini</span>
                    </div>
                    ...
                    <div class="flex justify-start">
                        <div
                            class="bg-gray-50 border border-gray-200 text-gray-800 px-5 py-3.5 rounded-2xl rounded-tl-sm max-w-[85%] shadow-sm">
                            <p class="text-sm font-medium leading-relaxed">Halo! Saya AI Botanist. Ada yang bisa saya bantu
                                terkait kondisi tanaman {{ $scanResult->plant_name ?? 'ini' }} Anda?</p>
                            <span
                                class="text-[10px] text-gray-400 mt-2 block font-bold tracking-wider">{{ now()->format('H:i') }}</span>
                        </div>
                    </div>
                        ...
                        @foreach ($scanResult->chatHistories as $chat)
                            @if ($chat->sender === 'user')
                                <div class="flex justify-end">
                                    <div
                                        class="bg-[#4C732E] text-white px-5 py-3.5 rounded-2xl rounded-tr-sm max-w-[85%] shadow-md">
                                        <p class="text-sm font-medium leading-relaxed">{{ $chat->message }}</p>
                                        <span
                                            class="text-[10px] text-green-200 mt-2 block text-right font-bold tracking-wider">{{ $chat->created_at->format('H:i') }}</span>
                                    </div>
                                </div>
                            @else
                                <div class="flex justify-start ai-history-bubble"
                                    data-message="{{ htmlspecialchars($chat->message) }}"
                                    data-time="{{ $chat->created_at->format('H:i') }}">
                                </div>
                            @endif
                        @endforeach
                    @endif
                ...
                <div
                    class="p-5 border-t border-gray-100 bg-white flex-shrink-0 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)]">
                    <div class="flex gap-3 items-end">
                        <textarea id="chat-input" rows="1" placeholder="Tanya soal tanamanmu..."
                            class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#4C732E]/30 focus:border-[#4C732E] resize-none custom-scrollbar font-medium"></textarea>
                        <button type="button" id="send-btn"
                            class="bg-[#4C732E] text-white p-3.5 rounded-xl hover:bg-[#3b5924] transition shadow-md flex-shrink-0 group">
                            <svg id="send-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                fill="currentColor" class="w-5 h-5 group-hover:scale-110 transition-transform">
                                <path
                                    d="M3.478 2.404a.75.75 0 0 0-.926.941l2.432 7.905H13.5a.75.75 0 0 1 0 1.5H4.984l-2.432 7.905a.75.75 0 0 0 .926.94 60.519 60.519 0 0 0 18.445-8.986.75.75 0 0 0 0-1.218A60.517 60.517 0 0 0 3.478 2.404Z" />
                            </svg>
                            <svg id="loading-icon" class="w-5 h-5 hidden animate-spin" xmlns="http://www.w3.org/2000/svg"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function getCurrentTime() {
                const now = new Date();
                return now.getHours().toString().padStart(2, '0') + ':' + now.getMinutes().toString().padStart(2,
                    '0');
            }
